...
const _fixedPeerHost = String.fromEnvironment('FIXED_PEER_HOST');
const _fixedPeerPortRaw = String.fromEnvironment('FIXED_PEER_PORT');

bool get _hasFixedPeer => _fixedPeerHost.trim().isNotEmpty;

int _fixedPeerPortFor(Network network) {
  final parsed = int.tryParse(_fixedPeerPortRaw.trim());
...
class _AppShell extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final tabs = <Tab>[
      const Tab(icon: Icon(Icons.storage), text: 'Block Storage'),
      if (!_hasFixedPeer)
...
class _LightNodePageState extends State<LightNodePage> {
  ...
  Future<void> _start() async {
    ...
    try {
      ...
      // 2. Discover a peer or connect to the fixed one if configured.
      final manager = PeerManager(network: _network, verbose: true);
      Peer? peer;
      if (_hasFixedPeer) {
        final ip = _fixedPeerHost.trim();
        final port = _fixedPeerPortFor(_network);
        setState(() {
          _state = _NodeRunState.connecting;
          _connectedPeer = '$ip:$port';
        });
        _addLog('Connecting to fixed peer $ip:$port…');
        peer = await manager.connectPeer(PeerCandidate(ip: ip, port: port));
      } else {
        setState(() => _state = _NodeRunState.discovering);
        _addLog('Searching for peer with compact filter support…');
        peer = await manager.findPeer();
      }
      if (_stopping || !mounted) return;
      ...
      if (peer == null) {
        ...
        _addLog(
          _hasFixedPeer
              ? 'Could not connect to fixed peer ${_fixedPeerHost.trim()}:${_fixedPeerPortFor(_network)}'
              : 'Could not find a peer with compact filter support',
        );
        return;
      }
      ...
      setState(() {
        _state = _NodeRunState.connecting;
        _connectedPeer = '${connectedPeer.ip}:${connectedPeer.port}';
      });
      _addLog('Found peer ✓  ${connectedPeer.ip}:${connectedPeer.port}');

      // Hand the already-handshaked peer to the node to start syncing.
      _node!.add(peer: connectedPeer);
